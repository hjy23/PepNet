...
def parse_args():
    ...
    parser.add_argument("--feature_file", "-feature_file", dest='feature_file', type=str,
                        default='../datasets/AIP/feature/test.h5',
                        help='The path of the test FASTA file.')
...
class Config():
    def __init__(self, args):
        ...
        if self.type == 'AMP':
            ...
            self.batch_size = 128
            self.hidden = 512
            self.model_time = '2024_08_02_10_47_29_952'
...
def data_pre(fasta_file, feature_file, theta=40, shuffle_seed=11, mode='train'):
    ...
    pre_feas = get_pretrained_features(names, sequences, feature_file, theta=theta)
...
def main(opt, device, shuffle_seed=13, split_seed=7, val_ratio=0.2):
    # feature_file = f'{opt.input_path}/{opt.feature_file}'
