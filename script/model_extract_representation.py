...
class AIMP(torch.nn.Module):
    ...
    def forward(self, pre_feas, feas):
        ...
        # return out[:, -1], transformer_out, feas_em.mean(dim=1), tcn_out.mean(dim=1), pre_feas.mean(dim=1), feas_raw.mean(dim=1)
        return out[:, -1], transformer_out, feas_em.mean(dim=1), tcn_out.mean(dim=1), pre_feas_0.mean(dim=1), feas_raw.mean(dim=1)
